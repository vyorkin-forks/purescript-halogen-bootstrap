...
main :: IO ()
main = shelly $ do
  styleSheet <- print_stdout False $ run "curl" ["https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0-alpha.4/css/bootstrap.min.css"]
  ...
  where
    ...
    escapeKeywords word =
      if word `elem` reserved then word <> "_" else word
      where
      reserved =
        [ "as", "case", "of", "class", "data", "default", "derive", "do", "forall", "foreign", "hiding", "if", "then", "else"
        , "import", "infix", "infixl", "infixr", "instance", "let", "in", "module", "newtype", "type", "where" ]
